...
char cmd_buff[100];
int8_t cmd_length = 0;
unsigned long interval=10000; // Thời gian delay
...
void loop() {
        ...
        Serial.println(ENV.temperatureC);
        if ((unsigned long)(currentMillis - previousMillis) >= interval) {
                if (connect_Server()) {
                        Serial.println(ENV.temperatureC);
                        get_data(ENV.temperatureC);
                }
