...
char cmd_buff[150];
int8_t cmd_length = 0;
unsigned long interval=58000; // Thời gian delay
...
void get_data(float number1) {
        memset(cmd_buff, '\0', 100);
        snprintf(cmd_buff, sizeof(cmd_buff), "GET %s%.2f", URL_current_temp, number1); //Gộp dữ liệu vào cmd_buff
        ...
        if (sendATcommand2(cmd_buff, ">", "ERROR", 5000) == 1) {
                snprintf(cmd_buff, sizeof(cmd_buff), "GET %s%.2f", URL_current_temp, number1);
