...
BLYNK_WRITE(V21) //Send data from app to hardware
{
        int pinValue = param.asInt(); // assigning incoming value from pin V1 to a variable
        //Serial.println(pinValue);
...
//        }
        if (year() != 1970) {
                ...
                Blynk.syncVirtual(V12); // sync scheduler #2
                Blynk.syncVirtual(V13); // sync scheduler #3
                Blynk.syncVirtual(V14); // sync scheduler #4
        }
        nowseconds = ((hour() * 3600) + (minute() * 60) + second());
