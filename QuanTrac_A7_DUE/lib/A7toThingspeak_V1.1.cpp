...
const unsigned long interval=30000; // Thời gian delay lấy mẫu tính theo milliseconds
...
// kết nối wifi thử tối đa 10 lần
        for (int8_t i = 0; i < 10; i++) { if (connect_Server()==1) { break; } }
}                                                                         // VÒNG LẶP VÔ HẠN
void loop() { // Gửi dữ liệu unsigned long currentMillis = millis(); // grab current time if ((unsigned long)(currentMillis - previousMillis) >= interval) {
...
// }
        get_data(t,h);
// // Kiểm tra
// if (h>=30&&h<=60) { // Serial.println("GOOD!. Độ ẩm khá thoải mái……"); // } else if (h>60) {
...
//snprintf(cmd_buff, sizeof(cmd_buff), "AT+CIPSEND"); //Gửi kết nối và chờ ">"
        ...
        if (sendATcommand2(cmd_buff, "OK", "ERROR", 5000) == 1) {
                ...
                if (sendATcommand2("AT+CIPSEND", ">", "ERROR", 5000) == 1) {
                        snprintf(cmd_buff, sizeof(cmd_buff), "GET %s%s%.2f%s%0.2f\n%c", URL_current_temp, "&field1=", number1,"&field2=", number2, 0x1A); // gửi 0x1A ký tự SUB sau ký tự xuống dòng
