...
char server[] = "192.168.68.120"; // Ip của Petrol 00-16-76-7b-72-03	 MAC
...
void loop() {
        ...
        /////////////////////////////////////////////////////////////////////////////
        if(client.find("<div><label>Tong ca - lit:</label><td><span id=\"pumdat_dlt\" style=\"font-weight:normal\"")) {
                //if(client.find("<div><label>Tien:</label><td>       ")) {
                //int Lit; // Kết quả mà trích xuất được
                if(client.find(">")) {
                        TongCalit = client.parseInt(SKIP_WHITESPACE);
        ...
        if(Dem == 2) {
                ...
                ThingSpeak.setField(2,(float) TongCalit);
                ThingSpeak.setField(3,(long) DonGia);
                int flag_Write = ThingSpeak.writeFields(myChannelNumber, myWriteAPIKey); // ERR code in thingspeak.h
                //Serial.print("Updated to thingspeak: " );
                //Serial.println(flag_Write);
                delay(5000);
                // if(flag_Write == ERR_CONNECT_FAILED || flag_Write == ERR_UNEXPECTED_FAIL) {
                if(flag_Write != OK_SUCCESS) {
                        software_Reset();
                }
                Dem = 0;
        }
